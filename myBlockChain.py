...
#리스트의 리스트를 객체로 변환
#txDataget.csv 에 writerows 하는 게 원하는 거야
...
def getTxData():
    strTxData = ''
    importedTx = readTx(g_txFileName) #readTx 함수 실행/ importedTx 는 리스트 안에 블록객체 정보를 담은 리스트를 가지고 있어
    if len(importedTx) > 0 :
        for i in importedTx:
            print(i.__dict__)
...
def mineNewBlock(difficulty=g_difficulty, blockchainPath=g_bcFileName):
    ...
    strTxData = getTxData() #문자열 위에서 데이터 다 합쳐준 거
    if strTxData == '' :
        print('No TxData Found. Mining aborted')
        return

    timestamp = time.time() #현재 블록의 첫 생성시도 시간
    proof = 0 # 작업증명 획수 초기화
    newBlockFound = False #while문 탈출조건

    print('Mining a block...')

    while not newBlockFound: #while True
        newBlockAttempt = generateNextBlock(blockchain, strTxData, timestamp, proof)
        if newBlockAttempt.currentHash[0:difficulty] == '0' * difficulty: #내가 셋팅한 난이도를 만족한다면
            #여기는 중요한 부분이 아니야
            #if 문의 주요목적은 newBlockFound 를 True 로 바꾸는 것
            # stopTime = time.time()
